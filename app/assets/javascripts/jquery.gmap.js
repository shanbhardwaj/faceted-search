 ...
 //                   image: opts.icon.image,
   ...
   //                 iconAnchor: new $googlemaps.Point(opts.icon.iconanchor[0], opts.icon.iconanchor[1]),
      ...
      //      if(marker.key) {$data.markerKeys[marker.key] = gmarker; }
          ...
          //      var infoContent = typeof(marker.html) === "string" ? opts.html_prepend + marker.html + opts.html_append : marker.html;
                ...
                $googlemaps.event.addListener(marker, 'mouseover', function () {
                	...
                	// }else{
	                	// methods.panToPosition(latlng);
	                	// $googlemaps.event.addListener(map,"idle",function(){
		            		// $("#mapdivwrapper").append("<div style='left: " + ($('#marker_' + m.text).offset().left - 155) + "px;top: " + ($('#marker_' + m.text).offset().top - 190) + "px' class='infowindow'  id='info' position: relative><div style='padding: 10px; margin-left: 10px; margin-top:10px'><div class='imagediv'><img src='"+m.image+"' style='width: 59px;height:60px'></div><div class='title'>"+m.title+"</div><div class='infowindowtext'>"+m.html +"</div><div class='tooltipbase'></div><div class='tooltip'></div></div>");	                		
	                	// });
            		// }            				
                });
        ...
        addMarkers: function (markers){
			...
			labeledmarker[i] = marker;
		        methods.processMarker.apply(this, [labeledmarker[i],markers[i], '/assets/red-dot.png', '/assets/shadow50.png', homeLatLng]);
	...
	addMarkerCustom: function(m){
		...
		methods.processMarker.apply(this, [marker,m, '/assets/red-dot.png', '/assets/shadow50.png', homeLatLng]);
		return this;
	},
	...
	showInfoWindow: function(id){
		// if(map.getBounds().contains(labeledmarker[m-1].getPosition()))
		var obj;
    	...
    	if(map.getCenter().equals(obj.getPosition())){
			google.maps.event.trigger(obj, 'mouseover');
    	}else{
        	methods.panToPosition(obj.getPosition());
        	$googlemaps.event.addListener(map,"idle",function(){
				google.maps.event.trigger(obj, 'mouseover');
        	});
		}            				
	...
	hideInfoWindow: function(id){
		...
		google.maps.event.trigger(obj,'mouseout');
		$googlemaps.event.clearListeners(map,'idle');
	},
	panMap: function(){
        if (labeledmarker.length > 0){
        	map.setZoom(Math.round(14-Math.log(1)/Math.LN2));
        	map.setCenter(labeledmarker[0].getPosition());
	   }
	},
    ...
    $.fn.gMap.defaults = {
        ...
        html_append:             '</div>',
        icon: {
            image:               "/assets/red-dot.png",
            iconsize:            [20, 34],
            iconanchor:          [9, 34],
            infowindowanchor:    [9, 2],
            shadow:              "/assets/shadow50.png",
            shadowsize:          [37, 34]
        },
